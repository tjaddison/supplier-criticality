  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatus('error');
        setMessage(`Failed to send request: ${errorData.message || 'Please try again.'}`);
      }
    } catch (error) {
      ...
      <main className="flex-1 py-16 md:py-24">
        <div className="container px-4 md:px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-[#194866] mb-4">
          ...
          <div className="max-w-xl mx-auto bg-white p-8 md:p-10 rounded-xl shadow-lg border border-gray-200">
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Name Field */}
              <div>
                <Label htmlFor="name" className="text-sm font-medium text-[#194866]">Full Name</Label>
              ...
              {/* Email Field */}
              <div>
                <Label htmlFor="email" className="text-sm font-medium text-[#194866]">Email Address</Label>
                 <div className="relative mt-1">
              ...
              {/* Company Field */}
              <div>
                <Label htmlFor="company" className="text-sm font-medium text-[#194866]">Company / Organization</Label>
                 <div className="relative mt-1">
              ...
              {/* <div>
                <Label htmlFor="message" className="text-sm font-medium text-[#194866]">Message (Optional)</Label>
                <Textarea
                  id="message"
                  name="message"
                  rows={4}
              ...
              <div>
                <Button
                  ...
                  className={`w-full font-bold text-lg py-3 px-6 bg-[${accentColor}] hover:bg-[#FF6A33] transition-all`}
                >
                  {status === 'loading' ? 'Sending...' : 'Request Demo / Send Message'}
                </Button>
              </div>

              {/* Status Message */}
              {message && (
                <p className={`text-sm text-center font-medium ${status === 'success' ? 'text-green-600' : 'text-red-600'}`}>
                  {message}
                </p>
              )}
            </form>
